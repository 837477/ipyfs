...
class NFTMetadataUploader:
    def __init__(
        ...
        self.ipfs_metadata_folder = os.path.join(ipfs_folder, metadata_folder)
    ...
    def check_folders(self):
        ...
        """
        try: self.files.stat(f'/{self.ipfs_folder}')
        except Exception as e:
            if e.args[0]['Code'] == 0:
                print(f'IPFS({self.ipfs_folder}) Folder does not exist. Creating...')
                self.files.mkdir(f'/{self.ipfs_folder}')

        try: self.files.stat(f'/{self.ipfs_images_folder}')
        except Exception as e: 
            if e.args[0]['Code'] == 0:
                print('Image folder does not exist. Creating...')
                self.files.mkdir(f'/{self.ipfs_images_folder}')
        
        try: self.files.stat(f'/{self.ipfs_metadata_folder}')
        except Exception as e: 
            if e.args[0]['Code'] == 0:
    ...
    def upload(self, create=True):
        ...
        for metadata_file in tqdm(all_metadata):
            with open(os.path.join(self.metadata_folder, metadata_file), "r") as f_meta:
                # Upload image to IPFS
                count_str = re.findall(r'(\d+)', metadata_file)[0]
                image_name = images[count_str]
                image_path = os.path.join(self.images_folder, image_name)
                with open(image_path, "rb") as f_image:
                    ipfs_f_image_path = os.path.join(self.ipfs_images_folder, image_name)
                    self.files.write(
                ...
                metadata['name'] = metadata['name'].format(count=int(count_str))
                metadata['image'] = metadata['image'].format(image_hash=info['result']['Hash'])
                ipfs_f_meta_path = os.path.join(self.ipfs_metadata_folder, metadata_file)
                self.files.write(
